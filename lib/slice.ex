...
defmodule H264.Decoder.Slice do
  ...
  def parse_single(data, bitOffset, nalResult, spsResult, ppsResult) do
    rest = data
    ...
    defaultResult = %{
      :field_pic_flag => 0, # ?
      :num_ref_idx_l0_active_minus1 => ppsResult[:num_ref_idx_l0_default_active_minus1],
      :num_ref_idx_l1_active_minus1 => ppsResult[:num_ref_idx_l1_default_active_minus1],
    }

    {result, rest, bitOffset} = {defaultResult, rest, bitOffset} |> parse_header(nalResult, spsResult, ppsResult)
      |> parse_data()
  ...
  defp parse_header({result, rest, bitOffset}, nalResult, spsResult, ppsResult) do
    nal_unit_type = nalResult[:nal_unit_type]
    # idr_pic_flag = nalResult[:idr_pic_flag]
    ...
    {result, rest, bitOffset} = {result, rest, bitOffset} |> BitReader.bit_read_ue_v(:first_mb_in_slice)
      ...
      |> BitReader.bit_cond_read(fn (_r) -> separate_colour_plane_flag == 1 end, fn args ->
        args |> BitReader.bit_read_u(:colour_plane_id, 2)
      end)
      |> BitReader.bit_read_u(:frame_num, log2_max_frame_num_minus4 + 4)
      |> BitReader.bit_cond_read(fn (_r) -> frame_mbs_only_flag == 0 end, fn args ->
      ...
      |> BitReader.bit_cond_read(fn (_r) -> pic_order_cnt_type == 0 end, fn args ->
        args |> BitReader.bit_read_u(:pic_order_cnt_lsb, log2_max_pic_order_cnt_lsb_minus4 + 4)
              |> BitReader.bit_func_read(fn {result, rest, bitOffset} ->
                field_pic_flag = result[:field_pic_flag]
                if bottom_field_pic_order_in_frame_present_flag == 1 and field_pic_flag == 0 do
      ...
      |> BitReader.bit_cond_read(fn _r -> pic_order_cnt_type == 1 and delta_pic_order_always_zero_flag == 0 end, fn args ->
        # {result, _r, _b} = args
      ...
      |> BitReader.bit_cond_read(fn _r -> redundant_pic_cnt_present_flag == 1 end, fn args ->
      ...
      |> BitReader.bit_cond_read(fn result ->
        ...
        or (weighted_bipred_idc == 1
          and (slice_type == @slice_type_all_b
                or slice_type == @slice_type_b))
      end, fn args ->
        args |> read_pred_weight_table(chromaArrayType)
      end)
      ...
      |> BitReader.bit_cond_read(fn _r -> deblocking_filter_control_present_flag == 1 end, fn args ->
      ...
      |> BitReader.bit_cond_read(fn _r ->
        (num_slice_groups_minus1 > 0) and (slice_group_map_type >= 3) and (slice_group_map_type <= 5)
      end, fn args ->
        args |> BitReader.bit_read_u(:slice_group_change_cycle, Float.ceil(Math.log2(picSizeInMapUnits / sliceGroupChangeRate + 1)))
      end)
  ...
  defp read_pred_weight_table(args, chromaArrayType) do
    {result, _r, _b} = args
    slice_type = result[:slice_type]
